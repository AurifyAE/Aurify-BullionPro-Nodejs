...
class MetalTransactionService {
  ...
  static buildImportPurchaseReturnUnfixEntries(
    ...
    // 2) HEDGE LOGIC
    // ----------------------------------------------------
    if (hedge && false && totals.pureWeight > 0) {
      if (totals.pureWeight > 0) {
      ...
      if (totals.pureWeightStd > 0) {
        entries.push(
          this.createRegistryEntry(
            ...
            "001",
            "HEDGE_ENTRY",
            `Hedge recorded for ${partyName} — ${totals.pureWeight}g at ${totals.bidValue}`,
            party._id,
            false,
            totals.pureWeight,
            totals.pureWeight,
    ...
    if (hedge && false && totals.pureWeight > 0) {
      ...
      entries.push(
        this.createRegistryEntry(
          ...
          "001",
          "HEDGE_ENTRY",
          `Hedge entry recorded for ${partyName} — ${totals.pureWeight}g at bid ${totals.bidValue}`,
          party._id,
          false,
          totals.pureWeight,
          totals.pureWeight,
  ...
  static calculateTotals(stockItems, totalSummary, isRegistry = false) {
    const totals = stockItems.reduce(
      (acc, item) => {
        ...
        console.log(pureWeight);
        console.log(pureWeightStd);
        ...
        //   console.log('+#+#+#+#+#+#+#+#+')
        //   console.log(finalPureWeight);
        //   console.log(finalPurity);
        //     console.log('+#+#+#+#+#+#+#+#+')
        // } else {
    ...
    if (Array.isArray(otherCharges) && otherCharges.length > 0) {
      for (const oc of otherCharges) {
        ...
        if (debit?.account && debit?.amountFC > 0) {
          ...
          logs.push(
            `🟢 DEBIT ${debit?.amountFC} (${cur}) → ${debit.account
            }`
          );
        }
        ...
        if (credit?.account && credit?.amountFC > 0) {
          ...
          logs.push(
            `🔴 CREDIT ${credit.amountFC.toFixed(2)} (${cur}) → ${credit.account
            }`
          );
        }
